...
function ContactChannelCard({
  ...
  return (
    <div
      className={`relative flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl bg-surface border border-border-subtle ${hoverBorderClass} transition-colors`}
    >
      <div className="flex items-center gap-3 min-w-0">
        <div
          className={`p-2.5 rounded-lg bg-surface border border-border-subtle ${iconColorClass} shrink-0`}
        >
          {icon}
        </div>
        ...
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-mono font-medium transition-all duration-200 border cursor-pointer ${
            copied
              ...
              : "bg-surface hover:bg-surface-hover text-text-primary border-border-subtle hover:border-accent-cyan/40"
          }`}
        >
          {copied ? (
            <>
      ...
      <AnimatePresence>
        {copied && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -4 }}
            transition={
              prefersReduced ? { duration: 0.1 } : motionTokens.toastSpring
            }
...
export function Footer() {
  ...
  return (
    <footer
      ...
      className="relative pt-12 sm:pt-16 pb-12 px-4 sm:px-6 lg:px-8 max-w-300 mx-auto w-full flex flex-col gap-10"
    >
      ...
      <div className="relative rounded-2xl border border-border-subtle bg-surface/70 backdrop-blur-md overflow-hidden shadow-xl p-6 sm:p-8 lg:p-10 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-3 max-w-xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-emerald/10 border border-accent-emerald/25 w-fit text-accent-emerald text-xs font-mono font-medium">
              <span className="relative flex h-2 w-2">
                {allowAmbientPulse && (
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-accent-emerald opacity-75 motion-reduce:hidden" />
                )}
        ...
        <div className="flex flex-col gap-4">
          <ContactChannelCard
            ...
            hoverBorderClass="hover:border-accent-cyan/30"
            toastLabel="Email copied to clipboard"
          ...
          <ContactChannelCard
            ...
            iconColorClass="text-accent-emerald"
            hoverBorderClass="hover:border-accent-emerald/30"
            toastLabel="Phone copied to clipboard"
            prefersReduced={prefersReduced}
            actionNode={
              <a
